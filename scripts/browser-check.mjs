...
try{
  ...
  await page.getByRole("button",{name:"Pan map",exact:true}).click();assert.equal(await page.getByRole("button",{name:"Pan map",exact:true}).getAttribute("aria-pressed"),"true");
  ...
  for(const locale of ["en","fr","ar"]){
    ...
    for(const width of [1440,1200,1024,768,430,390,320]){
      ...
      for(const route of ["/","/network","/how-it-works","/start-campaign?type=special","/contact","/privacy","/terms","/dashboard","/dashboard/profile",`/dashboard/${asset.campaign_id}`]){
        ...
        if(route==="/"||route==="/how-it-works"){assert.equal(await page.locator(".workflow-step").evaluateAll(items=>items.every(item=>item.getBoundingClientRect().width>=100)),true,`${locale} ${width} workflow columns`);if(width===390)await page.locator(".workflow").screenshot({path:`test-results/${locale}-${route==="/"?"home":"how"}-workflow.png`});}
  ...
  await touchPage.getByRole("button",{name:"Pan map",exact:true}).tap();await touchPage.locator(".map-viewport").scrollIntoViewIfNeeded();const pinch=await touchPage.locator(".map-viewport").boundingBox(),cx=pinch.x+pinch.width/2,cy=Math.max(180,Math.min(650,pinch.y+pinch.height/2));
